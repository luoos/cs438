...
// control message that wraps the distance vector sent by owner to its neighbors
class Message {
    public:
    int ownerId_;
...
void NetworkEmulator::setUpNodes() {
    // update Id range
    minId_ = INFINI, maxId_ = -1;
    for (auto &entry : nodes_) {
        if (entry.first < minId_) {
            minId_ = entry.first;
        }
        if (entry.first > maxId_) {
            maxId_ = entry.first;
        }
    }
    for (auto &entry : nodes_) {
        entry.second->updateIdRange(minId_, maxId_);
    }
}
...
void NetworkEmulator::resetAllNodes() {
    for (auto &entry : nodes_) {
        entry.second->reset();
    }
}
...
void NetworkEmulator::applyChange(tuple<int, int, int> change) {
    ...
    // distance vector can't handle broken link situation,
    // just reset all nodes to start over after any change
    resetAllNodes();
}
